...
# Context-budget controls. These prevent the prompt + answer from
# exceeding the existing 1536-token Mistral context window.
RAG_MAX_CONTEXT_CHUNKS = 3
RAG_MAX_CHUNK_CHARS = 1800
RAG_MAX_CONTEXT_CHARS = 4200
RAG_MIN_GENERATION_TOKENS = 48
...
# These events request cooperative cancellation without unloading
# the local LLM or changing the CUDA configuration.

is_busy = False

# Prevent delayed GUI callbacks from an older request from appearing
# inside a newer answer.
...
def run_ingestion(
    ...
    if (
        ...
        status_callback(
            "Document processing stopped before embedding."
        )
        return False
...
def normalize_source_name(value):
    """Normalize a filename/source for reliable case-insensitive matching."""
    return re.sub(
        r"\s+",
        " ",
...
def find_requested_sources(query):
    """
    Detect explicit local filenames mentioned in the user's query.

    Example:
        "According to India.pdf, what is India's capital?"
    -> ["India.pdf"]

    We use the actual indexed source names rather than inventing filenames.
    """
    query_norm = normalize_source_name(query)
    ...
    for item in chunks:
        ...
        if source_norm and source_norm not in seen:
...
def retrieve_context(
    ...
    """
    Retrieve local document context using FAISS.

    Important behavior:
    - FAISS remains the only vector retrieval engine.
    - Normalized embeddings + IndexFlatIP give cosine similarity.
    - If the user explicitly names an indexed file such as India.pdf,
      retrieval is restricted to that file. This prevents another
      document from winning the search merely because its embedding
      score is slightly higher.
    - No CUDA/LLM settings are changed here.
    """
    ...
    try:
        ...
        # --------------------------------------------------------
        # Detect an explicitly requested document.
        # --------------------------------------------------------

        requested_sources = find_requested_sources(query)

        # --------------------------------------------------------
        # STRICT EXPLICIT-SOURCE RULE
        #
        # If the user explicitly names a document, that document
        # MUST be indexed. Never silently search another document.
        # --------------------------------------------------------

        explicit_filename = bool(
            re.search(
                r"\b[^\s,?]+\.(?:pdf|docx|txt|wav)\b",
                query,
                flags=re.IGNORECASE
            )
        )

        if explicit_filename and not requested_sources:
            return [], (
        ...
        # --------------------------------------------------------
        # CASE 1:
        # User explicitly named an indexed document.
        #
        ...
        if requested_sources:
            ...
            source_index = faiss.IndexFlatIP(
                int(candidate_vectors.shape[1])
            )
            ...
            scores, local_indices = source_index.search(
                query_vector,
                search_k
            )
        ...
        # --------------------------------------------------------
        # CASE 2:
        # No explicit document name.
        #
        # Search the complete FAISS index normally.
        # --------------------------------------------------------
            ...
            for score, index in zip(
                ...
                raw_results.append((index, float(score)))
            ...
            if raw_results:
                ...
                for index, score in raw_results:
                    ...
                    if selected and score < best_score - 0.18:
                        continue
            ...
            if (
                len(selected) > 1
                and selected[0][1] >= 0.50
                and selected[1][1] < selected[0][1] - 0.08
            ):
                selected = selected[:1]
        ...
        #
        # The model context is 1536 tokens. Complete document chunks
        # can consume that window before answer generation begins.
        # Keep the strongest chunks and the most query-relevant text.
        # --------------------------------------------------------
        ...
        def compact_chunk(text_value):
            ...
            if len(raw) <= RAG_MAX_CHUNK_CHARS:
                return raw
            ...
            for overlap, order, piece in ranked:
        ...
        for index, score in selected_for_context:
...
def is_style_only_followup(query):
    ...
    patterns = [
        ...
        r"^(elaborate|elaborate more|expand|expand more)$",
        r"^(explain more|explain further|more details|more detail)$",
        r"^(explain clearly|clearly)$",
        r"^(explain simply|simplify|make it simple)$",
        r"^(in detail|explain in detail|detailed)$",
...
def remove_relevance_metadata(text):
    ...
    text = re.sub(
        r"\s*(?:with|at|having)\s+(?:a\s+)?relevance\s+score\s+of\s+[0-9.]+",
        "",
        text,
        flags=re.I
    )
    text = re.sub(
        r"\s*\(?relevance\s*[:=]\s*[0-9.]+\)?",
        "",
        text,
        flags=re.I
    )
    ...
    return clean_model_answer(text)


def build_answer_scope_instruction(query, style_followup=False, previous_topic=""):
...
The user is changing ONLY the answer style or length.
Stay strictly focused on that previous topic. Do not summarize
the whole document and do not introduce unrelated facts.

"briefly" means compress the previous answer.
"elaborate more" means expand the previous answer only with
relevant supporting facts.
"clearly" means rewrite the same answer more clearly.
"""
...
def strip_provenance_from_answer(answer):
    ...
    answer = re.sub(r"\s*📄\s*Sources?:.*$", "", answer, flags=re.I | re.S)
...
# ============================================================
# ANSWER STYLE / CONVERSATION CONTROLS
# ============================================================

DEFAULT_ANSWER_STYLE = """
Do not summarize the whole document unless explicitly requested.
Give a clear, useful answer at a consistent medium level of detail.
Normally answer in about 2-5 sentences: first give the direct answer,
then briefly explain the reason, meaning, or relevant detail.

Do not answer normal factual questions with only a single word unless
the user explicitly asks for a one-word answer.

Do not unnecessarily produce a long essay. Prefer concise paragraphs
with enough explanation to make the answer understandable.

If the question naturally requires steps, examples, comparison, or
multiple facts, use a small list when that makes the answer clearer.
"""

def detect_answer_style(query):
    """
    Detect explicit user requests to change answer length/detail.
    The default remains constant medium detail.
    """
    q = clean_text(query).lower()

    if re.search(
        r"\b(one word|one-word|single word|just the answer|only the answer)\b",
        q
    ):
        return "one_word"

    if re.search(
        r"\b(briefly|brief|in short|shortly|short answer|small sentence|"
        r"small explanation|very short|keep it short|concise)\b",
        q
    ):
        return "brief"

    if re.search(
        r"\b(elaborate more|elaborate|expand|explain more|more details|"
        r"in detail|detailed|explain in detail|deeply|deep explanation)\b",
        q
    ):
        return "detailed"

    if re.search(
        r"\b(clearly|explain clearly|clear explanation|simple explanation|"
        r"explain simply|simplify|easy explanation)\b",
        q
    ):
        return "clear"
...
def answer_style_instruction(query):
    ...
    if style == "one_word":
        return """
ANSWER LENGTH:
The user explicitly requested a one-word/only-answer response.
Give only the requested answer, with no explanation unless absolutely
...
Give 1-2 clear sentences. Keep the explanation minimal.
"""
...
Give a well-structured explanation with enough detail to fully teach
the point. Avoid unnecessary repetition.
"""

    if style == "clear":
        return """
ANSWER STYLE:
Explain the answer clearly and simply. Prefer plain language and
short sentences. Include enough explanation to make the idea easy
to understand.
"""
    ...
    if style == "steps":
        return """
ANSWER STYLE:
Explain the answer step by step when steps are appropriate. Keep
each step clear and useful rather than overly verbose.
"""

    if style == "summary":
        return """
ANSWER LENGTH:
The user requested a summary. Give the key points briefly and
clearly without unnecessary background.
"""
...
def clean_model_answer(answer):
    """
    Remove common prompt artifacts and accidental repeated labels
    without changing the factual content of the model's answer.
    """
    if not answer:
        return ""
    ...
    # Remove accidental Mistral instruction markers.
    answer = re.sub(r"^\s*\[/INST\]\s*", "", answer, flags=re.I)
    answer = re.sub(r"^\s*\[INST\]\s*", "", answer, flags=re.I)

    # Remove duplicated leading AI labels because the GUI already adds one.
    answer = re.sub(
        r"^\s*(?:🤖\s*)?(?:AI|Assistant)\s*:\s*",
        "",
        answer,
        flags=re.I
    )

    # Avoid repeated whitespace while preserving paragraph breaks.
...
{answer_style_instruction(query)}
...
{answer_scope_instruction(query, style_followup, last_answer_topic)}
...
- Never include FAISS similarity/relevance scores in the answer.
- Never include internal source labels such as "[Source: ...]" in the
  natural-language answer. The application displays sources separately.


- If the user asks a style-only follow-up such as "elaborate more",
  "briefly", "clearly", or "explain in detail", revise the previous
  answer using the previous question and its original local context.
  Do not treat the style phrase as a new factual search topic.
...
# ============================================================
# FIT PROMPT TO MODEL CONTEXT WINDOW
# ============================================================
...
def prepare_generation_request(
    ...
    """Fit prompt + output inside the existing 1536-token window."""
    ...
    for limit in limits:
        ...
        compact_context = original[:limit]
        ...
        if available >= RAG_MIN_GENERATION_TOKENS:
    ...
    # Last-resort prompt. This still preserves the current model
    # and CUDA configuration; only the generated answer budget shrinks.
    prompt = build_prompt(
        query,
        "",
        selected_language,
...
class RAGApp(ctk.CTk):
    ...
    def start_new_chat(self):
        ...
        if is_busy:
            self.lbl_status.configure(
                text="Stop the current generation first."
            )
            return
    ...
    def handle_enter_key(self, event=None):
        ...
        # One request at a time. While busy, Enter does nothing.
        if is_busy:
            return "break"
    ...
    def toggle_send_mic_button(
        ...
        # While the local model is generating, the single action
        # button becomes STOP. Enter/Send is also blocked by
        # start_generation(), so multiple requests cannot overlap.
        if is_busy:
    ...
    def stream_token(
        ...
        """Append a token only for the currently active request."""

        global generation_request_id
    ...
    def stop_processing(self):
        ...
        self.lbl_status.configure(
            text="Stopping document processing..."
        )
    ...
    def start_processing(self):
        ...
        def worker():
            ...
            try:
                ...
                if processing_stop_event.is_set():
                    ...
                    if not processing_stop_event.is_set():
    ...
    def stop_generation(self):
        """Request a cooperative stop without unloading the LLM."""
    ...
    def start_generation(self):
        ...
        global last_answer_core
        global last_rag_context
        global last_selected_retrieval
        global last_answer_topic
        global is_busy
        global generation_request_id
        ...
        # Hard single-request rule. Repeated clicks and Enter are ignored
        ...
        # ChatGPT-like single active request: do not allow another
        # prompt to be edited/submitted while the current generation runs.
        self.entry_query.configure(
            state="disabled"
        )
        ...
        def worker():
            ...
            try:
                ...
                if (
                    generation_stop_event.is_set()
                    or request_id != generation_request_id
                ):
                    return
                ...
                style_followup = is_style_only_followup(query)

                if (
                    style_followup
                    and last_user_query
                    and last_answer_core
                    and last_rag_context
                ):
                    # A style command is not a new factual search.
                    selected = list(last_selected_retrieval)
                    context = (
                        "PREVIOUS ANSWER:\n"
                        + last_answer_core
                    ...
                    selected, context = retrieve_context(query)
                ...
                if generation_stop_event.is_set():
                    ...
                    last_response_text = "⏹ Generation stopped."
                    return
                ...
                if (
                    is_document_grounded_question(
                        last_user_query if style_followup else query
                    )
                    and not selected
                ):
                    answer = (
                ...
                # ====================================================
                # Every language is streamed so STOP works consistently.
                # The existing loaded Mistral/CUDA instance is reused.
                # ====================================================
                ...
                answer = clean_model_answer(full_answer)
                ...
                # The GUI owns provenance labels. Remove any labels
                # accidentally generated by Mistral.
                answer = strip_provenance_from_answer(answer)
                answer = remove_relevance_metadata(answer)
                ...
                if retrieved_sources:
                    ...
                    # Include provenance in speech/history state too.
                    last_response_text = answer

                # Save only completed answers as conversation memory.
                # "elaborate more", "briefly", etc. reuse this memory.
                if not stopped:
                    if not style_followup:
                        last_user_query = query
                        last_answer_topic = query

                    last_answer_core = remove_relevance_metadata(
                        strip_provenance_from_answer(answer)
                    )

                    if not style_followup:
                        last_rag_context = context if selected else ""

                    last_selected_retrieval = list(selected)
                    last_response_text = answer
                else:
                    last_response_text = answer
...
if __name__ == "__main__":
    ...
    print("RAG: FAISS source-aware retrieval enabled")
    print("UI: Single-request Send/Stop control enabled")
